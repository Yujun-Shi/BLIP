...
def get(data_path,seed=0,pc_valid=0.15, fixed_order=False):
    data={}
    taskcla=[]
    size=[3,32,32]
    ...
    path = os.path.join(data_path, 'binary_mixture_5')
    if not os.path.isdir(path):
        ...
        for n,idx in enumerate(idata):
            if idx==0:
                ...
                std=(0.4501,)
                dat={}
                dat['train']=notMNIST(os.path.join(data_path, 'notmnist'), train=True, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=notMNIST(os.path.join(data_path, 'notmnist'), train=False, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                ...
                for s in ['train','test']:
                    ...
                    for image, target in loader:
                        data[n][s]['x'].append(image)
                        # initially, there is a weriod target.numpy()[0]-1, but I suspect it's because change of dataset
                        data[n][s]['y'].append(target.numpy()[0])
                ...
                std=(0.3318,)
                dat={}
                dat['train']=FashionMNIST(os.path.join(data_path, 'fashion_mnist'), train=True, download=True, transform=transforms.Compose([
                    transforms.Pad(padding=2, fill=0), transforms.ToTensor(),transforms.Normalize(mean, std)]))
                dat['test']=FashionMNIST(os.path.join(data_path, 'fashion_mnist'), train=False, download=True, transform=transforms.Compose([
                    transforms.Pad(padding=2, fill=0), transforms.ToTensor(),transforms.Normalize(mean, std)]))
                data[n]={}
