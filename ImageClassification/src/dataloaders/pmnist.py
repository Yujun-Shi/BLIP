...
def get(data_path, seed=0, fixed_order=False, pc_valid=0):
    ...
    dir_path = os.path.join(data_path, 'binary_pmnist')
    if not os.path.isdir(dir_path):
        ...
        for i, r in enumerate(seeds):
            ...
            for s in ['train', 'test']:
                ...
                torch.save(data[i][s]['x'],os.path.join(os.path.expanduser(dir_path), 'data' + str(r) + s + 'x.bin'))
                torch.save(data[i][s]['y'],os.path.join(os.path.expanduser(dir_path), 'data' + str(r) + s + 'y.bin'))
        print()
        ...
        for i, r in enumerate(seeds):
            ...
            for s in ['train', 'test']:
                data[i][s] = {'x': [], 'y': []}
                data[i][s]['x'] = torch.load(os.path.join(os.path.expanduser(dir_path), 'data' + str(r) + s + 'x.bin'))
                data[i][s]['y'] = torch.load(os.path.join(os.path.expanduser(dir_path), 'data' + str(r) + s + 'y.bin'))
